...
def relu_adf_layer(params: dict[str, Any], relu_layer: nn.ReLU) -> nn.Module:
    ...
    layer_fn = getattr(adf_layers, layer)
    relu_adf_layer = layer_fn(
        **params,
    )
    return relu_adf_layer

def leakyrelu_adf_layer(params: dict[str, Any], leakyrelu_layer: nn.LeakyReLU) -> nn.Module:
...
def dropout_adf_layer(params: dict[str, Any], dropout_layer: nn.Dropout) -> nn.Module:
    ...
    layer_fn = getattr(adf_layers, layer)
    dropout_adf_layer = layer_fn(
        p = dropout_layer.p,
        **params,
    )
    return dropout_adf_layer

def maxpool2d_adf_layer(params: dict[str, Any], maxpool2d_layer: nn.MaxPool2d) -> nn.Module:
    """Convert maxpool2d deterministic layer to maxpool2d adf layer."""
    
    layer = maxpool2d_layer.__class__.__name__ + "_adf"
    layer_fn = getattr(adf_layers, layer)
    maxpool2d_adf_layer = layer_fn(
        **params,
    )
    return maxpool2d_adf_layer

def avgpool2d_adf_layer(params: dict[str, Any], avgpool2d_layer: nn.AvgPool2d) -> nn.Module:
    """Convert avgpool2d deterministic layer to avgpool2d adf layer."""
    
    layer = avgpool2d_layer.__class__.__name__ + "_adf"
    layer_fn = getattr(adf_layers, layer)
    avgpool2d_adf_layer = layer_fn(
        **params,
    )
    return avgpool2d_adf_layer

def batchnorm2d_adf_layer(params: dict[str, Any], batchnorm2d_layer: nn.BatchNorm2d) -> nn.Module:
...
def conv_adf_layer(params: dict[str, Any], conv_layer: Union[nn.Conv2d, nn.ConvTranspose2d]) -> nn.Module:
...
def convert_deterministic_to_adf(
