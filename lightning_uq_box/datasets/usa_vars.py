...
from torchgeo.datasets import NonGeoDataset, USAVars
    ...
    def __init__(
        ...
        if split == "ood":
            if ood_type == "tail":
                ...
                assert (
                    ood_range[0] >= self.in_target_max
                ), "OOD min should be larger than in distribution max."
                self.ood_range = ood_range

        # rebalance
        if self.ood_type == "tail":
            self.in_dist_df = self.full_df[
                self.full_df["treecover"] < self.in_target_max
            ].reset_index(drop=True)
        else:
            self.in_dist_df = self.full_df[
                (self.full_df["treecover"] < 33.333)
                | (self.full_df["treecover"] > 66.666)
            ].reset_index(drop=True)
        num_total = len(self.in_dist_df[self.in_dist_df["treecover"] < 10])
        discard = self.in_dist_df[self.in_dist_df["treecover"] < 10].sample(
            n=num_total - 5000
        )
        ...
        if split == "train":
            self.feature_df = self.in_dist_df[
                self.in_dist_df.index.isin(self.train_ids)
            ]
        elif split == "val":
            ...
            if self.ood_type == "tail":
                self.feature_df = self.ood_df[
                    (self.ood_df["treecover"] >= ood_range[0])
                    & (self.ood_df["treecover"] <= ood_range[1])
                ]
            else:
...
class USAVarsOOD(USAVars):
    ...
    def __init__(
        ...
        self.in_dist_set = self.label_df[
            self.label_df["treecover"] < self.in_target_max
        ]
