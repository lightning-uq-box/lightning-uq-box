...
class DDPM(BaseModule):
    ...
    * https://arxiv.org/abs/2006.11239
    ...
    def p_sample(self, x: Tensor, t: int, cond_variables: Tensor | None = None) -> tuple[Tensor, Tensor]:
    ...
    @torch.inference_mode()
    def p_sample_loop(self, shape: tuple[int, ...], return_all_timesteps: bool = False, cond_variables: Tensor | None = None) -> Tensor:
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            img, x_start = self.p_sample(img, t, cond_variables)
            imgs.append(img)
    ...
    @torch.inference_mode()
    def sample(self, batch_size: int = 16, return_all_timesteps: bool = False, cond_variables: Tensor | None = None) -> Tensor:
        ...
        batch_size = cond_variables.shape[0] if cond_variables is not None else batch_size
        samples = self.p_sample_loop((batch_size, channels, h, w), return_all_timesteps=return_all_timesteps, cond_variables=cond_variables)
        self.use_ema_model = False
        return samples
    ...
    def model_predictions(self, x, t, cond_variables: Tensor | None = None, clip_x_start = False):
        """Model predictions for the given input."""
        if self.use_ema_model:
            model_output = self.ema.ema_model(x, t, cond_variables)
        else:
            model_output = self.model(x, t, cond_variables)
    ...
    def p_mean_variance(self, x, t, cond_variables: Tensor | None = None, clip_denoised = True):
        preds = self.model_predictions(x, t, cond_variables)
    ...
    def p_losses(self, x_start, t, cond_variables: torch.Tensor = None, noise = None, offset_noise_strength = None):
        ...
        # TODO: check how to support self-conditioning
        # x_self_cond = None
        ...
        model_out = self.model(x, t, x_self_cond=cond_variables)
    ...
    def encode_img(self, latent_model: nn.Module, img: Tensor, up_sample: bool=False) -> Tensor:
    ...
    def training_step(
        ...
        """
        X, cond = batch[self.input_key], batch.get('conditions', None)
        ...
        loss = self.p_losses(X, t, cond_variables = cond)
    ...
    def log_samples(self, preds: Tensor, inputs: None | Tensor, targets: None | Tensor) -> None:
