...
def test_invalid_inputs_are_rejected() -> None:
    maps = torch.zeros(2, 4, 4, dtype=torch.long)
    with pytest.raises(ValueError, match="num_classes >= 2"):
        iou_with_empty_convention(maps, maps, 1)
    with pytest.raises(ValueError, match=r"\[0, num_classes\)"):
        iou_with_empty_convention(maps + 3, maps, 2)
    with pytest.raises(ValueError, match="spatial dimensions"):
        iou_with_empty_convention(maps, torch.zeros(2, 4, 5, dtype=torch.long), 2)
    with pytest.raises(ValueError, match="matching batch sizes"):
        generalized_energy_distance(
            torch.zeros(2, 3, 4, 4, dtype=torch.long),
