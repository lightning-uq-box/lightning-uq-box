...
def test_binary_segmentation_prediction_is_not_degenerate() -> None:
    ...
    ProbUNet accepts ``task="binary"``, which produces single-channel logits. Routing those through the multiclass branch
...
def test_save_preds_false_writes_nothing() -> None:
    ...
    it/s. Both the directory and the write are now gated on it.
    """
    import tempfile
...
def test_decoder_without_channel_count_rejected() -> None:
    """A decoder with no layer exposing out_channels cannot size Fcomb."""
    import pytest

    class NoWidthDecoder(nn.Module):
    ...
    with pytest.raises(ValueError, match="could not infer"):
        ProbUNet(model=NoWidthDecoder(), latent_dim=6, num_samples=2)


def test_decoder_taking_stages_as_separate_arguments() -> None:
    """Older smp decoders take the encoder stages as positional arguments."""
    ...
    class StarDecoder(nn.Module):
        ...
        def forward(self, *features: torch.Tensor) -> torch.Tensor:
            return self.conv(features[-1])

    class OldStyleUnet(nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.encoder = Stages()
    ...
    method = ProbUNet(model=OldStyleUnet(), latent_dim=6, num_samples=2)
    features = method.extract_features(torch.randn(2, 3, 16, 16))
    assert features.shape == (2, 8, 16, 16)
